...
// Get recent activities (requires activity:read_all scope)
export async function getRecentActivities(limit = 10) {
  try {
    ...
    console.warn('⚠️ Cannot fetch activities - requires activity:read_all scope:', error.message);
    // Return empty array since we don't have permission
    return [];
  }
}
...
export async function getStravaDashboardData() {
  try {
    ...
    console.log('📊 Activities available:', activities.length > 0 ? `Yes (${activities.length})` : 'No');

    // Calculate metrics from available data
    let summary, activityTypes, weeklyPattern, recentActivities;
    ...
    if (activities.length > 0) {
      ...
      activityTypes = recentActivities.reduce((types, activity) => {
      ...
      // No activity data available, use stats data if available
      console.log('⚠️ No activity data available - using stats data');
      recentActivities = [];
      ...
      if (stats) {
        ...
        activityTypes = {};
        if (recentRideTotals.count > 0) activityTypes['Ride'] = recentRideTotals.count;
        if (recentRunTotals.count > 0) activityTypes['Run'] = recentRunTotals.count;

        weeklyPattern = {}; // No weekly pattern available from stats
      } else {
        // No stats either, create minimal summary
        summary = {
          totalActivities: 0,
          totalDistance: 0,
          totalTime: 0,
    ...
    const dashboardData = {
      ...
      dataSource: activities.length > 0 ? 'activities' : 'stats'
    };
